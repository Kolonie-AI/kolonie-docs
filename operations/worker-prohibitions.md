...
| **A person's judgement** | An issue carrying `decision`, or an open trade-off with two defensible answers. The prompt tells the model to make the smallest defensible change; an issue whose whole content *is* the choice has no smallest one | `AGENTS.md` §5, and the refusal shape `#250` distinguishes from a rule refusal |
