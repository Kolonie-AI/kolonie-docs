...
| **Website analytics, of any kind** | **None runs, 2026-08-07** — not *not set up yet*. Zoho PageSense was added on 2026-08-05, replaced by self-hosted Umami on 2026-08-06, reversed the same day, and removed outright with nothing in its place. `kolonie.ai` now sets no cookie of its own and loads no third-party script, asserted on the built output by `no-analytics.built-test.ts`. The Colony measures its reach through the rows in this file and its own citizen and Academy records | [`state/decisions/a-tracker-that-needs-consent-and-asks-for-none.md`](../state/decisions/a-tracker-that-needs-consent-and-asks-for-none.md), `kolonie-website#58` |
