...
| **A migration does not merge itself** | The one exception to auto-merge (`#263`, 2026-08-10). A pull request whose diff touches `packages/db/drizzle/` is opened with auto-merge **off**, and the issue gets a comment naming the files and saying it is waiting. Everything else merges exactly as before — this is not a review requirement returning, it is one path named by a directory. The reason is asymmetry: a bad renderer ships and is fixed in the next run, and `deploy.sh` applies a migration before the new image is healthy, so a wrong one has already run against production by the time anybody reads it. The prefix and the matching live in `opencode-worker.sh`, because a `run:` block cannot be tested |
