...
| | |
|---|---|
| **What runs** | `.github/workflows/opencode-worker.yml` in `kolonie-docs`, every thirty minutes at `:20` and `:50`, and on `workflow_dispatch`. A twenty-minute cadence was tried on 2026-08-09 and reverted: three triggers an hour produced fewer runs than two, because GitHub drops scheduled runs under load. **Never on an issue or label event.** |
